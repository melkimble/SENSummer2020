...
hist_words <- function(DTM, plotTitle='', tidyTrim=FALSE) {
  ret_list <- list()
  ...
  if (tidyTrim) {
    ...
    print(paste("tidytrimming:",plotTitle))
    ...
    ret_list[['DTM_trimmed']]<-DTM_trimmed
  }
  
  plt<-ggplot(DTM_words, aes(x = nchar)) +
    ggtitle(plotTitle) +
...
full_DTM<-hist_words(full_DocTermMatrix, "Publications")
full_DTM[['plt']]

# iterate through hist_words function
#for (i in 1:length(DTM_list)) {
  
#  gplt<-hist_words(DTM_list[[i]], DTM_names[i],tidyTrim=TRUE)
#  print(DTM_names[i])

#  DTM_terms_list[[i]]<-gplt[[1]]
#  DTM_trimmed_list[[i]]<-gplt[[2]]  
#  gtab_plt_list[[i]]<-gplt[[3]]
...
```{r}
word_assoc <- findAssocs(full_DTM[['DTM_terms']], 'ken', 0.6)
